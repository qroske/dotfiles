...
return {
  ...
  config = function()
    ...
    -- ---------------------------------------------------------
    -- Enterキーの挙動カスタマイズ
    -- 補完メニューが表示時と通常の改行かで動作を分ける
    -- ---------------------------------------------------------
    vim.keymap.set("i", "<CR>", function()
      -- pumvisible(): 補完メニューが表示されているか？
      -- 0 = 非表示、1 = 表示中
      if vim.fn.pumvisible() ~= 0 then
        return vim.api.nvim_replace_termcodes("<C-y>", true, true, true) -- Ctrl+yで補完を確定
      else
        return autopairs.autopairs_cr() -- autopairsの改行。括弧の間で改行すると自動インデント
      end
    end, {
